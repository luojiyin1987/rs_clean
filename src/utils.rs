...
use which::which; // 引入 ConfigError
...
pub fn validate_exclude_dir_name(dir_name: &str) -> Result<(), ConfigError> {
    ...
    if dir_name.contains("..") || dir_name.contains('/') || dir_name.contains('\\') {
        return Err(ConfigError::InvalidConfig(format!(
            "Invalid exclude directory name: '{}'",
            dir_name
        )));
    }
    ...
    if dir_name == "." || dir_name == ".." {
        return Err(ConfigError::InvalidConfig(format!(
            "Reserved directory name cannot be excluded: '{}'",
            dir_name
        )));
    }
    ...
    if windows_reserved.contains(&dir_name_lower.as_str()) {
        return Err(ConfigError::InvalidConfig(format!(
            "Windows reserved name cannot be used as exclude directory: '{}'",
            dir_name
        )));
    }
